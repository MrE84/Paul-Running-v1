...
export default function ActivityWorkspace({ projection: p, units, loadRaw, loadWeather }: { projection: AnalysisProjection; units: UnitSystem; loadRaw: () => Promise<DecodedFit>; loadWeather?: () => Promise<AnalysisProjection> }) {
  ...
  return <>
    <header className={styles.activityHeader}>
      ...
      <div className={styles.headerTools}><Link href={`/activity-analysis/compare?id=${encodeURIComponent(p.activity.id)}`}>Add to comparison</Link><Link href="/activity-analysis/trends">Performance trends</Link><span className={styles.badge}>{p.quality.samples.toLocaleString()} samples · {available.length} channels</span></div>
    </header>
    ...
    <WorkspaceLayoutControls sport={p.activity.sport} availableChannels={available} layout={layout} onChange={onLayoutChange} />
    <nav className={styles.tabs} aria-label="Analysis views">{visibleViews.map(v => <button key={v} aria-pressed={view === v} className={view === v ? styles.activeTab : ""} onClick={() => setView(v)}>{v}</button>)}</nav>
    ...
    {view === "Intervals / laps" && <AnalysisPanelBoundary activityId={p.activity.id} name="Intervals and laps"><IntervalsPanel projection={p} intelligence={intelligence} selection={selection} onSelect={onSelect} units={units} /></AnalysisPanelBoundary>}
    {view === "Zones" && <AnalysisPanelBoundary activityId={p.activity.id} name="Zones and relationships"><ZonesPanel projection={p} selection={selection} onSelect={onSelect} units={units} /></AnalysisPanelBoundary>}
    {view === "Best efforts" && <AnalysisPanelBoundary activityId={p.activity.id} name="Best efforts"><BestEffortsPanel projection={p} intelligence={intelligence} selection={selection} onSelect={onSelect} units={units} /></AnalysisPanelBoundary>}
    {view === "Raw data" && <AnalysisPanelBoundary activityId={p.activity.id} name="Raw data">{raw ? <RawData decoded={raw} name={p.source.name} /> : <section className={styles.panel} role="status">{rawLoading ? "Loading complete FIT data…" : rawError || "Opening raw data…"}{rawError && <button onClick={() => { setView(visibleViews.find(value => value !== "Raw data") ?? "Raw data"); }}>Return to analysis</button>}</section>}</AnalysisPanelBoundary>}
    <details className={styles.quality}><summary>Data quality, corrections & export <span>{p.quality.flags.length + anomalies.length ? `${p.quality.flags.length + anomalies.length} notes` : "All available streams ready"}</span></summary>
      <div className={styles.qualityActions}><label><input type="checkbox" checked={corrections} disabled={!correctionLayers.length} onChange={event => setCorrections(event.target.checked)} />Interpolate flagged signal samples for display</label><button onClick={downloadPrivateGpx}>Download privacy-safe GPX</button></div>
