...
export class InMemoryTrainingApiStore implements TrainingApiStore {
  private readonly athletes = new Map<string, TrainingApiSeed["athletes"] extends (infer T)[] ? T : never>();
  ...
  constructor(seed: TrainingApiSeed = {}) {
    ...
    for (const revision of seed.workoutRevisions ?? []) {
      this.workoutRevisions.set(revisionKey(revision.workoutId, revision.version), revision);
    }
    for (const workout of seed.workouts ?? []) {
      this.workoutRevisions.set(
        revisionKey(workout.id, workout.currentRevision.version),
        workout.currentRevision,
      );
    }
    for (const plan of seed.plans ?? []) this.plans.set(plan.id, plan);
    for (const revision of seed.planRevisions ?? []) {
      this.planRevisions.set(revisionKey(revision.planId, revision.version), revision);
    }
    for (const plan of seed.plans ?? []) {
      this.planRevisions.set(revisionKey(plan.id, plan.currentRevision.version), plan.currentRevision);
    }
  ...
  async listCapacities(athleteId: string) {
    return this.capacities.filter((item) => item.athleteId === athleteId)
      .sort((a, b) => a.effectiveFrom.localeCompare(b.effectiveFrom));
  }
  async listZoneSets(athleteId: string) {
    return this.zones.filter((item) => item.athleteId === athleteId)
      .sort((a, b) => a.effectiveFrom.localeCompare(b.effectiveFrom));
  }

  async listWorkouts(athleteId: string) {
    return [...this.workouts.values()].filter((item) => item.athleteId === athleteId)
      .sort((a, b) => a.id.localeCompare(b.id));
  }
  async getWorkout(id: string) { return this.workouts.get(id); }
  async getWorkoutRevision(id: string, version: number) {
    return this.workoutRevisions.get(revisionKey(id, version));
  }
  async saveWorkout(workout: Workout, revision: WorkoutRevision) {
    this.workouts.set(workout.id, workout);
    this.workoutRevisions.set(revisionKey(workout.id, revision.version), revision);
  }

  async listPlans(athleteId: string) {
    return [...this.plans.values()].filter((item) => item.athleteId === athleteId)
      .sort((a, b) => a.id.localeCompare(b.id));
  }
  async getPlan(id: string) { return this.plans.get(id); }
  async getPlanRevision(id: string, version: number) {
    return this.planRevisions.get(revisionKey(id, version));
  }
  async savePlan(plan: TrainingPlan, revision: TrainingPlanRevision) {
    this.plans.set(plan.id, plan);
    this.planRevisions.set(revisionKey(plan.id, revision.version), revision);
  }
  ...
  async savePlanApplication(application: PlanApplication, items: ScheduledCalendarItem[]) {
    this.applications.set(application.id, application);
    for (const item of items) this.calendar.set(item.id, item);
  }

  async listActivities(athleteId: string, limit = 20) {
