...
export const ANALYSIS_ALGORITHM_VERSION = "2026-09-batch-a";
...
export function activityListItem(item: Omit<Activity, "normalizedData">): ActivityListItem {
  return {
    ...
    distance: item.summary.distanceMeters ?? null,
    duration: item.summary.durationSeconds ?? null,
...
function projectZones(zoneSets: ZoneSet[], sport: string, startedAt: string | null): AnalysisProjection["zones"] {
  ...
  for (const set of valid) {
    ...
    zones[key] = set.zones.filter(z => expected.includes(z.unit.toLowerCase())).map((z, i) => ({
      name: z.name,
      lower: safeNumber(z.lowerBound),
      upper: safeNumber(z.upperBound),
...
export function projectActivity(source: ActivitySource, decoded: DecodedFit, context?: ActivityListItem, zoneSets: ZoneSet[] = []): AnalysisProjection {
  ...
  const latitude: Values = [];
  const longitude: Values = [];
  ...
  rows.forEach(({ r, seconds }, i) => {
    ...
    if (d !== null && d < lastDistance) {
      d = null;
      flags.add("Distance resets excluded from distance alignment.");
    }
    ...
    const values: Record<Channel, number | null> = {
      ...
      power: nonnegative(safeNumber(r.power)),
      temperature: safeNumber(r.temperature),
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (d !== null && d0 !== null && d - d0 >= 10) {
      if (channels.grade[i] === null && h !== null && h0 !== null) {
        channels.grade[i] = round(100 * (h - h0) / (d - d0));
        derivedGrade = true;
      }
      anchor = i;
    }
  ...
  const activity: ActivityListItem = context ?? {
    id: source.id,
    title: String(session.sport_profile_name || source.name.replace(/\.fit$/i, "")),
    sport: String(session.sport || "other"),
    startedAt,
    distance: summary.distance,
    duration: summary.timerTime,
  };
  ...
  const projectedLaps = laps.map((lap, i): AnalysisLap => {
    ...
    return {
      id: `lap-${i + 1}`,
      label: `Lap ${i + 1}`,
      start: nearestIndex(elapsed, startSeconds),
      end: nearestIndex(elapsed, lapElapsed),
      duration,
      distance: safeNumber(lap.total_distance),
      heartRate: safeNumber(lap.avg_heart_rate),
    };
  });
  ...
  return {
    ...
    source: { id: source.id, name: source.name, origin: source.origin },
    activity,
    summary: {
      distance: summary.distance ?? activity.distance,
      duration: summary.timerTime ?? activity.duration,
      elapsed: summary.elapsedTime ?? elapsed.at(-1) ?? null,
      speed: summary.avgSpeed,
      heartRate: safeNumber(session.avg_heart_rate),
      cadence: summary.avgCadence,
      ascent: summary.totalAscent,
      calories: safeNumber(session.total_calories),
    },
    streams: { elapsed, distance, latitude, longitude, breakBefore, channels: available, recordIndex: rows.map(r => r.index) },
    laps: projectedLaps,
    zones,
    provenance: {
      sourceFields: [...summary.recordFields].sort(),
      algorithms: { projection: PROJECTION_VERSION, algorithm: ANALYSIS_ALGORITHM_VERSION, ...(derivedGrade ? { grade: "elevation-delta-10m-v1" } : {}) },
    },
